...
import type { TaskRequest as TaskCreateRequest, TaskResponse, Priority, TaskStatus as Status } from "@/lib/api";
type TaskUpdateRequest = Partial<Omit<TaskCreateRequest, "studentId">>;
import { tasksApi, tryApi, isAuthenticated } from "@/lib/api";
...
export function Tasks({ openCreateOnMount = false, onConsumed }: { openCreateOnMount?: boolean; onConsumed?: () => void } = {}) {
  ...
  React.useEffect(() => {
    (async () => {
      if (isAuthenticated()) {
        const remote = await tryApi<TaskResponse[]>(tasksApi.list() as Promise<TaskResponse[]>);
  ...
  async function handleCreate(payload: TaskCreateRequest | TaskUpdateRequest) {
    const p = payload as TaskCreateRequest;
    const remote = isAuthenticated() ? await tryApi<TaskResponse>(tasksApi.create(p) as Promise<TaskResponse>) : null;
    const now = new Date().toISOString();
    const newTask = (remote ?? {
  ...
  async function handleUpdate(id: string, payload: TaskCreateRequest | TaskUpdateRequest) {
    const u = payload as TaskUpdateRequest;
    if (isAuthenticated()) {
      try { await tasksApi.update(id, u); }
      catch (err: any) { toast.error("Impossible de mettre à jour la tâche"); }
    }
  ...
  async function handleDelete(id: string) {
    if (isAuthenticated()) {
      ...
      catch (err: any) { toast.error("Impossible de supprimer la tâche"); }
    }
