...
import { AlertTriangle, Trash2, MessageSquare, ChevronRight } from "lucide-react";
import { reportsApi, tryApi, isAuthenticated } from "@/lib/api";
...
export function ReportPage() {
  ...
  React.useEffect(() => {
    ...
    if (isAuthenticated()) {
      (async () => {
        try {
          const remote = await reportsApi.mine();
          if (remote && Array.isArray(remote)) {
          // Merge remote into local for display
          const mapped: Report[] = remote.map((r: any) => ({
            id: r.id ?? crypto.randomUUID(),
            category: (r.category as Report["category"]) ?? "OTHER",
            subject: r.subject ?? r.title ?? "(sans sujet)",
            description: r.description ?? r.message ?? "",
            status: (r.status as Report["status"]) ?? "OPEN",
            createdAt: r.createdAt ?? new Date().toISOString(),
            userEmail: r.userEmail ?? r.email ?? "",
            reply: r.reply ?? r.adminReply ?? undefined,
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (isAuthenticated()) {
      try {
        await reportsApi.create(`[${category}] ${subject.trim()}\n${description.trim()}`);
      } catch (err: any) {
        toast.error("Impossible d'envoyer le signalement au serveur");
      }
    }
    const r: Report = {
              ...
              <div key={r.id} className="rounded-2xl bg-surface-2 p-4 shadow-[0_4px_24px_rgba(79,110,247,.08)]">
                ...
                >
                  <div className="flex items-center gap-2 font-semibold">
                    <ChevronRight size={14} className={"transition-transform " + (openId === r.id ? "rotate-90" : "")} />
                    {r.subject}
                    {r.reply && (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      ...
                      className="inline-flex h-7 w-7 cursor-pointer items-center justify-center rounded-lg text-muted-foreground hover:bg-[color-mix(in_oklab,var(--danger)_15%,transparent)] hover:text-[var(--danger)]"
                    >
                      <Trash2 size={13} />
                    </span>
                  </div>
                </button>
                <div className="mt-1 text-xs text-muted-foreground">{CATS.find((c) => c.v === r.category)?.label} · {new Date(r.createdAt).toLocaleString()}</div>
                {openId === r.id && (
                  <>
                    <div className="mt-2 text-sm">{r.description}</div>
                    <div className="mt-3 rounded-lg border border-border bg-surface p-3">
                      <div className="mb-1 flex items-center gap-1.5 text-[11px] font-bold uppercase text-brand">
                        <MessageSquare size={12} /> Réponse de l'administration
                      </div>
                      {r.reply ? (
                        <>
                          <div className="text-sm">{r.reply}</div>
                          {r.repliedAt && <div className="mt-1 text-[10px] text-muted-foreground">{new Date(r.repliedAt).toLocaleString()}</div>}
                        </>
                      ) : (
                        <div className="text-xs text-muted-foreground italic">⏳ En attente de réponse de l'administration…</div>
                      )}
                    </div>
                  </>
                )}
