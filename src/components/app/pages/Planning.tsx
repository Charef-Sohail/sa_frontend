...
import { calendarApi, plannerApi, tryApi, isAuthenticated } from "@/lib/api";
...
export function Planning() {
  ...
  const [view, setView] = React.useState<"today" | "week" | "month">("today");
  ...
  async function onPickFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (isAuthenticated()) {
      try {
        const res = await calendarApi.importIcs(f);
        if (res) toast.success("Calendrier synchronisé avec le serveur");
      } catch (err: any) {
    ...
    <div>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          onClick={async () => {
              if (isAuthenticated()) {
                try { await plannerApi.generate(); toast.success(t("toast.optimized")); }
                catch (err: any) { toast.error("Impossible de générer le planning"); }
              } else {
                toast.info(t("toast.optimized"));
              }
            }}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_280px]">
        <div className="rounded-2xl bg-surface-2 p-5 shadow-[0_4px_24px_rgba(79,110,247,.08)]">
          ...
          <div className="mb-2.5 text-xs font-bold text-muted-foreground">
            {view === "today" && "Lundi 9 Juin 2026"}
            {view === "week" && "Semaine du 8 au 14 Juin 2026"}
            {view === "month" && "Juin 2026"}
          </div>
          {view === "today" && (
          ...
          {view === "week" && (
            <div className="grid grid-cols-7 gap-2">
              {["Lun","Mar","Mer","Jeu","Ven","Sam","Dim"].map((d, i) => (
                <div key={d} className="rounded-xl bg-surface p-3">
                  <div className="mb-2 text-[11px] font-bold text-muted-foreground">{d} {8+i}</div>
                  <div className="space-y-1.5">
                    {(i < 5 ? slots.slice(0, (i % 3) + 1) : []).map((s) => {
                      const p = palette[s.tone as keyof typeof palette];
                      return (
                        <div key={s.time}
                          className="rounded-md px-2 py-1 text-[10px] font-semibold"
                          style={{ background: p.bg, borderInlineStart: `2px solid ${p.border}` }}>
                          {s.time} · {s.title.replace(/^[^ ]+ /, "")}
                        </div>
                      );
                    })}
                    {i >= 5 && <div className="text-[10px] italic text-muted-foreground">Repos</div>}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-7 gap-1">
                {Array.from({ length: 30 }).map((_, i) => {
                  const day = i + 1;
                  const busy = [2,5,9,12,15,18,22,25,28].includes(day);
                  const exam = day === 10;
                  return (
                    <div key={day} className="aspect-square rounded-lg bg-surface p-1.5 text-[10px]">
                      <div className="font-bold">{day}</div>
                      {exam && <div className="mt-0.5 truncate rounded bg-danger/15 px-1 text-[9px] font-semibold text-danger">Examen</div>}
                      {busy && !exam && <div className="mt-0.5 truncate rounded bg-brand-light px-1 text-[9px] font-semibold text-brand">{Math.ceil(Math.random()*3)} tâches</div>}
                    </div>
                  );
                })}
              </div>
