...
import { adminApi, surveyApi, tryApi, isAuthenticated, auth, isTokenValid, type SurveyQuestion } from "@/lib/api";
...
/* ---------------- Page principale ---------------- */
type Section = "dashboard" | "users" | "reports" | "markets" | "faqs" | "survey" | "settings";
...
function AdminPage() {
  ...
  React.useEffect(() => {
    ...
    if (!isTokenValid() || !auth.isAdmin()) {
      // clear any partial state and redirect to login
      auth.clear();
      navigate({ to: "/login" });
    }
  }, []);
  ...
  const items: { key: Section; label: string; Icon: React.ComponentType<{ size?: number }> }[] = [
    ...
    { key: "survey", label: "Questions Survey", Icon: ListChecks },
    ...
    <RequireAdmin>
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-5 sm:p-7">
          ...
          {section === "survey" && <SurveyAdminPanel />}
...
function UsersPanel() {
  ...
  React.useEffect(() => {
    ...
    tryApi<any[]>(adminApi.users() as Promise<any[]>).then((remote) => {
      if (remote && Array.isArray(remote) && remote.length) {
        const mapped: AdminUser[] = remote.map((u: any) => ({
  ...
  function remove(id: string) {
    ...
    if (isAuthenticated()) {
      try { adminApi.deleteUser(id); } catch (e) { toast.error("Impossible de supprimer l'utilisateur"); }
    }
  ...
  function toggleStatus(id: string) {
    ...
    if (u && isAuthenticated()) {
      try { adminApi.updateUser(id, { status: u.status === "ACTIVE" ? "SUSPENDED" : "ACTIVE" }); }
      catch (e) { toast.error("Impossible de changer le statut"); }
    }
  ...
  function submitForm(payload: Omit<AdminUser, "id" | "createdAt">) {
    if (editing) {
      if (isAuthenticated()) {
        try { adminApi.updateUser(editing.id, payload); }
        catch (e) { toast.error("Impossible de modifier l'utilisateur"); }
      }
...
function ReportsPanel() {
  ...
  React.useEffect(() => {
    ...
    if (isAuthenticated()) {
      tryApi<any[]>(adminApi.reports() as Promise<any[]>).then((remote) => {
        if (remote && Array.isArray(remote) && remote.length) {
          const mapped: UserReport[] = remote.map((r: any) => ({
            id: r.id ?? crypto.randomUUID(),
            category: (r.category as UserReport["category"]) ?? "OTHER",
            subject: r.subject ?? r.title ?? (r.message?.slice(0, 60) ?? "(sans sujet)"),
            description: r.description ?? r.message ?? "",
            status: (r.status as UserReport["status"]) ?? "OPEN",
  ...
  function remove(id: string) {
    ...
    if (isAuthenticated()) {
      try { adminApi.deleteReport(id); }
      catch (e) { toast.error("Impossible de supprimer le signalement"); }
    }
  ...
  function setStatus(id: string, status: UserReport["status"]) {
    if (isAuthenticated()) {
      try { adminApi.replyReport(id, `[status:${status}]`); }
      catch (e) { toast.error("Impossible de changer le statut"); }
    }
  ...
  async function sendReply(id: string) {
    ...
    if (text.length < 2) { toast.error("Réponse trop courte"); return; }
    if (isAuthenticated()) {
      try { await adminApi.replyReport(id, text); }
      catch (e) { toast.error("Échec de l'envoi de la réponse"); }
    }
    const now = new Date().toISOString();
    persist(list.map((r) => r.id === id ? { ...r, reply: text, repliedAt: now, status: "RESOLVED" } : r));
    setReplyDraft((d) => ({ ...d, [id]: "" }));
    toast.success("Réponse envoyée à l'étudiant ✅");
  }
            ...
            {filtered.map((r) => (
              <tr key={r.id} className="border-b border-border last:border-0 align-top">
                <Td>
                  ...
                  {r.reply && (
                    <div className="mt-2 rounded-lg border border-border bg-surface p-2 text-[11px]">
                      <span className="font-bold text-brand">↪ Réponse :</span> {r.reply}
                    </div>
                  )}
                  <div className="mt-2 flex gap-1">
                    <input
                      ...
                      placeholder={r.reply ? "Modifier la réponse…" : "Répondre à l'étudiant…"}
                      className="flex-1 rounded-md border border-border bg-surface px-2 py-1 text-[11px] outline-none focus:border-brand"
                    />
                    <button
                      onClick={() => sendReply(r.id)}
                      className="inline-flex items-center gap-1 rounded-md bg-brand px-2 py-1 text-[11px] font-bold text-white hover:bg-brand-dark"
                    >
...
/* ---------------- Survey Admin (questions) ---------------- */
const SURVEY_Q_KEY = "sc-admin-survey-questions";

function seedSurveyQuestions(): SurveyQuestion[] {
  return [
    { id: crypto.randomUUID(), text: "À quelle heure vous réveillez-vous habituellement ?", type: "TEXT" },
    { id: crypto.randomUUID(), text: "Quel est votre pic de productivité ?", type: "CHOICE", options: ["MORNING", "AFTERNOON", "EVENING", "NIGHT"] },
    { id: crypto.randomUUID(), text: "Durée maximale de concentration (minutes) ?", type: "NUMBER" },
  ];
}
...
function SurveyAdminPanel() {
  ...
  const [open, setOpen] = React.useState(false);
  const [editing, setEditing] = React.useState<SurveyQuestion | null>(null);

  React.useEffect(() => {
    setQuestions(loadOrSeed<SurveyQuestion>(SURVEY_Q_KEY, seedSurveyQuestions));
    if (isAuthenticated()) {
      (async () => {
        try {
          const remote = await surveyApi.questions();
          if (remote && Array.isArray(remote) && remote.length) setQuestions(remote);
        } catch (e) { /* keep local */ }
      })();
    }
  }, []);

  function persist(next: SurveyQuestion[]) { setQuestions(next); save(SURVEY_Q_KEY, next); }

  async function remove(id: string) {
    if (!confirm("Supprimer cette question ?")) return;
    if (isAuthenticated()) {
      try { await surveyApi.deleteQuestion(id); }
      catch (e) { toast.error("Impossible de supprimer la question"); }
    }
    persist(questions.filter((q) => q.id !== id));
    toast.success("Question supprimée");
  }

  async function submit(payload: Omit<SurveyQuestion, "id">) {
    if (editing) {
      if (isAuthenticated()) {
        try { await surveyApi.updateQuestion(editing.id, payload); }
        catch (e) { toast.error("Impossible de modifier la question"); }
      }
      persist(questions.map((q) => q.id === editing.id ? { ...q, ...payload } : q));
      toast.success("Question modifiée");
    } else {
      let created: SurveyQuestion | null = null;
      if (isAuthenticated()) {
        try { created = await surveyApi.createQuestion(payload); }
        catch (e) { toast.error("Impossible de créer la question"); }
      }
      const newQ: SurveyQuestion = created ?? { id: crypto.randomUUID(), ...payload };
  ...
  return (
    <div>
      <PanelHeader title="Questions du questionnaire" emoji="📋"
        cta={<button onClick={() => { setEditing(null); setOpen(true); }} className="inline-flex items-center gap-1.5 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white hover:bg-brand-dark"><Plus size={16} /> Ajouter</button>}
      />
      <div className="flex flex-col gap-3">
        {questions.map((q, i) => (
          <div key={q.id} className="rounded-2xl bg-surface-2 p-4 shadow-[0_4px_24px_rgba(79,110,247,.08)]">
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1">
                <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Question {i + 1} · {q.type}</div>
                <div className="mt-1 font-semibold">{q.text}</div>
                {q.options && q.options.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {q.options.map((o) => (
                      <span key={o} className="rounded-full bg-brand-light px-2 py-0.5 text-[10px] font-bold text-brand">{o}</span>
                    ))}
                  </div>
                )}
              </div>
              <div className="flex gap-1">
                <IconBtn onClick={() => { setEditing(q); setOpen(true); }} label="Modifier"><Pencil size={14} /></IconBtn>
                <IconBtn danger onClick={() => remove(q.id)} label="Supprimer"><Trash2 size={14} /></IconBtn>
              </div>
            </div>
          </div>
        ))}
        {questions.length === 0 && (
          <div className="rounded-2xl bg-surface-2 p-6 text-center text-sm text-muted-foreground">Aucune question. Ajoutez-en une.</div>
        )}
      </div>
      <SurveyQuestionDialog open={open} initial={editing ?? undefined} onClose={() => { setOpen(false); setEditing(null); }} onSubmit={submit} />
    </div>
  );
}

function SurveyQuestionDialog({ open, initial, onClose, onSubmit }: {
  open: boolean; initial?: SurveyQuestion; onClose: () => void;
  onSubmit: (q: Omit<SurveyQuestion, "id">) => void;
}) {
  const [text, setText] = React.useState("");
  const [type, setType] = React.useState<SurveyQuestion["type"]>("TEXT");
  const [optionsStr, setOptionsStr] = React.useState("");
  React.useEffect(() => {
    if (!open) return;
    setText(initial?.text ?? "");
    setType(initial?.type ?? "TEXT");
    setOptionsStr((initial?.options ?? []).join(", "));
  }, [open, initial]);
  if (!open) return null;
  const needOptions = type === "CHOICE" || type === "MULTI";
  return (
    <Modal title={initial ? "Modifier la question" : "Nouvelle question"} onClose={onClose}>
      <form onSubmit={(e) => {
        e.preventDefault();
        if (text.trim().length < 3) { toast.error("Texte trop court"); return; }
        const options = needOptions
          ? optionsStr.split(",").map((s) => s.trim()).filter(Boolean)
          : undefined;
        if (needOptions && (!options || options.length < 2)) { toast.error("Au moins 2 options"); return; }
        onSubmit({ text: text.trim(), type, options });
      }}>
        <Input label="Texte de la question" value={text} onChange={setText} />
        <div className="mb-4">
          <label className="mb-1.5 block text-xs font-semibold">Type de réponse</label>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {(["TEXT", "CHOICE", "MULTI", "NUMBER"] as const).map((t) => (
              <button key={t} type="button" onClick={() => setType(t)}
                className={"rounded-lg border-[1.5px] px-3 py-2 text-xs font-semibold " + (type === t ? "border-brand bg-brand text-white" : "border-border bg-surface-2")}>
                {t}
              </button>
            ))}
          </div>
        ...
        {needOptions && (
          <Input label="Options (séparées par des virgules)" value={optionsStr} onChange={setOptionsStr} />
        )}
        <FormActions onCancel={onClose} submitLabel={initial ? "Enregistrer" : "Créer"} />
      </form>
    </Modal>
